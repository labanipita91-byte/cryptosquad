...
async function init() {
    const { data: p } = await supabaseClient.from('profiles').upsert([{ user_id: MY_ID, user_name: MY_NAME, chat_id: CHAT_ID }]).select().single();
...
async function renderPortfolio() {
    ...
    list.innerHTML = assets.map(a => {
        ...
        return `<div class="asset-item"><div><strong>${a.coin_id.toUpperCase()}</strong><br><small>${a.amount}</small></div>
                <div style="text-align:right"><strong>$${v.toLocaleString()}</strong><br><small class="${c>=0?'up':'down'}">${c>=0?'↑':'↓'} ${Math.abs(c).toFixed(2)}%</small></div></div>`;
    }).join('');
...
async function renderSquadLists() {
    ...
    document.getElementById('created-list').innerHTML = created?.map(s => `<div class="squad-link" onclick="loadLeaderboard('${s.id}')">${s.name} ➔</div>`).join('') || "None";
    document.getElementById('joined-list').innerHTML = jd?.filter(j => j.owner_id !== MY_ID).map(j => `<div class="squad-link" onclick="loadLeaderboard('${j.id}')">${j.name} ➔</div>`).join('') || "None";
}
...
async function loadLeaderboard(gid) {
    ...
    document.getElementById('sq-code-display').innerText = "Code: " + group.invite_code;
...
window.copyCode = () => { navigator.clipboard.writeText(document.getElementById('sq-code-display').innerText.split(': ')[1]); alert("Copied!"); };
window.shareSquad = () => tg.openTelegramLink(`https://t.me/share/url?url=https://t.me/CryptoSquadProBot?startapp=${document.getElementById('sq-code-display').innerText.split(': ')[1]}`);
